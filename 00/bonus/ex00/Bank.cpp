...
Bank::Account::Account() : m_id(-1), m_value(0) {}

Bank::Account::Account(int p_id, int p_value) : m_id(p_id), m_value(p_value) {}

Bank::Account::Account(const Account& p_other) : m_id(p_other.m_id), m_value(p_other.m_value) {}

Bank::Account& Bank::Account::operator=(const Account& p_other)
{
	if (this != &p_other)
	{
...
int const& Bank::Account::GetId() const
{
	return this->m_id;
}

int const& Bank::Account::GetValue() const
{
	return this->m_value;
}
...
int const& Bank::GetLiquidity() const
{
...
std::ostream& operator<<(std::ostream& p_os, const Bank& p_bank)
{
	...
	for (std::map<int, Bank::Account*>::const_iterator it = p_bank.m_database.begin(); it != p_bank.m_database.end(); ++it)
